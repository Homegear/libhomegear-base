...
#include "TranslationFileParser.h"

namespace BaseLib {

std::unordered_map<std::string, std::unordered_map<std::string, std::string>> TranslationFileParser::parse(const std::string &filepath) {
  ...
  for (auto &directory : languageDirectories) {
    ...
    languages.emplace(language, std::move(translations));
  }
