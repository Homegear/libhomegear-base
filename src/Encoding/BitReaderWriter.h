...
public:
	...
	/**
	 * Sets any number of bits at any position in a byte array. The source bytes are read LSB first (i. e. least significant byte at the smallest index). For example 0xABCD in source becomes 0xCDAB in target.
	 *
	 ...
	 * @param source The right aligned byte array of the data to set. Bytes are read MSB first by default and set at position in target (counted from the most significant bit of the most significant byte).
	 */
	 ...
	 * @param size The size in bits of the data to set.
	 * @param target The byte array to write to. Index 0 is the most significant byte.
	 * @param source The right aligned byte array of the data to set. Bytes are read LSB first by default and set at position in target (counted from the most significant bit of the most significant byte).
	 */
	static void setPositionLE(uint32_t position, uint32_t size, std::vector<char>& target, const std::vector<uint8_t>& source);

    /**
     * Sets any number of bits at any position in a byte array. The source bytes are read MSB first (i. e. most significant byte at the smallest index). For example 0xABCD in source stays 0xABCD in target.
     *
     * @param position The position in bits starting with bit 7 of index 0 (position 1 is bit 6 of index 0 and so on). Example: If source is 00000101, position is 2, size is 3 and target.size() is 2 then the result is 00101000 00000000.
     * @param size The size in bits of the data to set.
     * @param target The byte array to write to. Index 0 is the most significant byte. When target is larger than ceil(size / 8.0) bytes, the last (bytes with higher index) ceil(size / 8.0) bytes are used.
     * @param source The right aligned byte array of the data to set. Bytes are read LSB first by default and set at position in target (counted from the most significant bit of the most significant byte).
     */
