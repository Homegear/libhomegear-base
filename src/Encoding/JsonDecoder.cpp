...
void JsonDecoder::decodeObject(const std::string &json, uint32_t &pos, std::shared_ptr<Variable> &variable) {
  ...
  while (pos < json.length()) {
    ...
    if (!posValid(json, pos)) throw JsonDecoderException("No closing '}' found.");
    if (json[pos] != ':') {
      variable->arrayValue->push_back(std::make_shared<Variable>(name)); //Store property name in array to be able to access the object in the original order
      variable->structValue->insert(StructElement(name, std::make_shared<Variable>()));
      if (json[pos] == ',') {
        pos++;
        skipWhitespace(json, pos);
        if (!posValid(json, pos)) throw JsonDecoderException("No closing '}' found.");
...
std::string JsonDecoder::decodeString(const std::string &s) {
  ...
  for (int32_t i = 0; i < (signed)s.size(); i++) {
    ...
    if (c == '\\') {
      ...
      c = s[i];
      switch (c) {
        case 'b':utf8.push_back('\b');
          break;
        case 'f':utf8.push_back('\f');
          break;
        case 'n':utf8.push_back('\n');
          break;
        case 'r':utf8.push_back('\r');
          break;
        case 't':utf8.push_back('\t');
          break;
        case 'u': {
          i += 4;
          if (!posValid(s, i)) break;
...
bool JsonDecoder::decodeValue(const std::string &json, uint32_t &pos, std::shared_ptr<Variable> &value) {
  ...
  switch (json[pos]) {
    case 'n':decodeNull(json, pos, value);
      break;
    case 't':decodeBoolean(json, pos, value);
      break;
    case 'f':decodeBoolean(json, pos, value);
      break;
    case '"':decodeString(json, pos, value);
      break;
    case '{':decodeObject(json, pos, value);
      break;
    case '[':decodeArray(json, pos, value);
      break;
    default: {
...
bool JsonDecoder::decodeNumber(const std::string &json, uint32_t &pos, std::shared_ptr<Variable> &value) {
  ...
  } else return false; //Invalid number => interpret as string
...
bool JsonDecoder::decodeNumber(const std::vector<char> &json, uint32_t &pos, std::shared_ptr<Variable> &value) {
  value->type = VariableType::tInteger;
  if (!posValid(json, pos)) return false;
  bool minus = false;
  if (json[pos] == '-') {
