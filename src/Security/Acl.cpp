...
AclResult Acl::checkNodeBlueVariableReadAccess(const std::string &nodeId, int32_t input) {
  try {
    AclResult variableResult = AclResult::notInList;
    if (_variablesReadSet) {
      auto variablesIterator = _variablesRead.find(0x50000001); //Check specific access first in case of "no access".
      if (variablesIterator == _variablesRead.end()) variablesIterator = _variablesRead.find(0x50000000);
      if (variablesIterator != _variablesRead.end()) {
        auto channelIterator = variablesIterator->second.find(input); //Check specific access first in case of "no access".
        if (channelIterator != variablesIterator->second.end()) {
          auto variableIterator = channelIterator->second.find(nodeId); //Check specific access first in case of "no access".
    ...
    if (variableResult == AclResult::accept) return AclResult::accept;
...
AclResult Acl::checkNodeBlueVariableWriteAccess(const std::string &nodeId, int32_t input) {
  try {
    AclResult variableResult = AclResult::notInList;
    if (_variablesWriteSet) {
      auto variablesIterator = _variablesWrite.find(0x50000001); //Check specific access first in case of "no access".
      if (variablesIterator == _variablesWrite.end()) variablesIterator = _variablesWrite.find(0x50000000);
