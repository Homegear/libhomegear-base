...
bool Acls::checkNodeBlueVariableReadAccess(const std::string &nodeId, int32_t input) {
  try {
    if (nodeId.empty() || input < 0) return false;
    std::lock_guard<std::mutex> aclsGuard(_aclsMutex);
    bool acceptSet = false;
    for (auto &acl : _acls) {
      auto result = acl->checkNodeBlueVariableReadAccess(nodeId, input);
      if (result == AclResult::error || result == AclResult::deny) {
        if (_bl->debugLevel >= 5) _out.printDebug("Debug: Access denied to Node-BLUE variable of node " + nodeId + " (1).");
        return false;
      } else if (result == AclResult::accept) acceptSet = true;
    }

    if (!acceptSet && _bl->debugLevel >= 5) _out.printDebug("Debug: Access denied to Node-BLUE variable of node " + nodeId + " (2).");
    return acceptSet;
  }
  catch (const std::exception &ex) {
    _out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
...
bool Acls::checkNodeBlueVariableWriteAccess(const std::string &nodeId, int32_t input) {
  try {
    if (nodeId.empty() || input < 0) return false;
    std::lock_guard<std::mutex> aclsGuard(_aclsMutex);
    bool acceptSet = false;
    for (auto &acl : _acls) {
      auto result = acl->checkNodeBlueVariableWriteAccess(nodeId, input);
      if (result == AclResult::error || result == AclResult::deny) {
        if (_bl->debugLevel >= 5) _out.printDebug("Debug: Access denied to Node-BLUE variable of node " + nodeId + " (1).");
        return false;
      } else if (result == AclResult::accept) acceptSet = true;
