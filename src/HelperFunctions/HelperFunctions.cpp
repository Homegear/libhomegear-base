...
PVariable HelperFunctions::xml2variable(const xml_node *node, bool &isDataNode) {
  ...
  for (xml_node *subNode = node->first_node(); subNode; subNode = subNode->next_sibling()) {
    ...
    auto subNodeVariable = xml2variable(subNode, isDataNode2);
    ...
    Struct::iterator variableIterator;
    if (nodeVariable->type == VariableType::tStruct) {
      variableIterator = nodeVariable->structValue->find(nodeName);
      if (variableIterator == nodeVariable->structValue->end()) {
        if (isDataNode2) {
          auto valueStruct = std::make_shared<Variable>(VariableType::tStruct);
    ...
    if (nodeVariable->type == VariableType::tArray || changeTypeToArray) {
      //Multiple data nodes with same name? => Create array
      if (nodeVariable->type != VariableType::tArray) {
        nodeVariable->structValue->clear();
        nodeVariable->type = VariableType::tArray;
      }
      auto valueStruct = std::make_shared<Variable>(VariableType::tStruct);
      valueStruct->structValue->emplace(nodeName, subNodeVariable);
      nodeVariable->arrayValue->emplace_back(valueStruct);
