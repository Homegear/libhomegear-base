...
class RpcConfigurationParameter {
 public:
  ...
  bool hasCategory(uint64_t id) {
    ...
    return _categories.find(id) != _categories.end();
  }
  void addCategory(uint64_t id) {
    std::lock_guard<std::mutex> categoriesGuard(_categoriesMutex);
    _categories.emplace(id);
  }
  ...
  Role getRole(uint64_t id) {
    std::lock_guard<std::mutex> rolesGuard(_rolesMutex);
    auto rolesIterator = _roles.find(id);
    if (rolesIterator != _roles.end()) return rolesIterator->second; else return Role();
  }
...
class Peer : public ServiceMessages::IServiceEventSink, public IEvents {
 public:
  ...
  virtual void setFirmwareVersionString(std::string value) {
    _firmwareVersionString = value;
    saveVariable(1003, value);
  }
  ...
  virtual void saveSpecialTypeParameter(uint32_t parameterID,
