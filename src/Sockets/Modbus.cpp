...
Modbus::Modbus(BaseLib::SharedObjects *baseLib, Modbus::ModbusInfo &serverInfo) {
  ...
  _readBuffer = std::make_unique<std::vector<char>>(1024);
...
std::vector<char> Modbus::getResponse(std::vector<char> &packet) {
  ...
  else if (_readBuffer->at(7) & 0x80) //Error response
  {
    ...
    switch (exceptionCode) {
      ...
      case 2:throw ModbusException("Exception code 2: Illegal data address.", exceptionCode, response);
      case 3:throw ModbusException("Exception code 3: Illegal data value.", exceptionCode, response);
      case 4:throw ModbusException("Exception code 4: Server failure.", exceptionCode, response);
      case 5:
        throw ModbusException(
            "Exception code 5: Acknowledge: The server accepted the service invocation but the service requires a relatively long time to execute. The server therefore returns only an acknowledgement of the service invocation receipt.",
            exceptionCode,
            response);
      case 6:throw ModbusServerBusyException("Exception code 6: Server busy", exceptionCode, response);
      case 10:throw ModbusException("Exception code 10: Gateway problem: Gateway paths not available.", exceptionCode, response);
      case 11:throw ModbusException("Exception code 11: Gateway problem: The targeted device failed to respond.", exceptionCode, response);
      default:throw ModbusException("Unknown Modbus exception: " + std::to_string(exceptionCode) + ". Response was: " + BaseLib::HelperFunctions::getHexString(response), exceptionCode, response);
    }
  }
...
void Modbus::writeMultipleCoils(uint16_t startAddress, const std::vector<uint8_t> &values, uint16_t coilCount) {
  ...
  for (int32_t i = 0; i < 5; i++) {
    try {
      response = getResponse(packet);
      if (response.size() == 12 && response.at(8) == (char)(uint8_t)(startAddress >> 8) && response.at(9) == (char)(uint8_t)(startAddress & 0xFF) && response.at(10) == (char)(uint8_t)(coilCount >> 8)
          && response.at(11) == (char)(uint8_t)(coilCount & 0xFF))
        break;
...
void Modbus::writeMultipleRegisters(uint16_t startAddress, const std::vector<uint16_t> &values, uint16_t registerCount) {
  ...
  for (int32_t i = 0; i < 5; i++) {
    try {
      response = getResponse(packet);
      if (response.size() == 12 && response.at(8) == (char)(uint8_t)(startAddress >> 8) && response.at(9) == (char)(uint8_t)(startAddress & 0xFF) && response.at(10) == (char)(uint8_t)(registerCount >> 8)
          && response.at(11) == (char)(uint8_t)(registerCount & 0xFF))
        break;
  ...
  while (true) {
    ...
    if (exception || response.size() < 8 || response.at(7) & 0x80) //Exception
    {
      ...
      for (int32_t i = 0; i < numberOfObjects; i++) {
        ...
        switch (objectId) {
          case 0:deviceInfo.vendorName = std::string((char *)data.data(), data.size());
            break;
          case 1:deviceInfo.productCode = std::string((char *)data.data(), data.size());
            break;
          case 2:deviceInfo.majorMinorRevision = std::string((char *)data.data(), data.size());
            break;
          case 3:deviceInfo.vendorUrl = std::string((char *)data.data(), data.size());
            break;
          case 4:deviceInfo.productName = std::string((char *)data.data(), data.size());
            break;
          case 5:deviceInfo.modelName = std::string((char *)data.data(), data.size());
            break;
          case 6:deviceInfo.userApplicationName = std::string((char *)data.data(), data.size());
            break;
        }
