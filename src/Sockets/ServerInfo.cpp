...
PVariable ServerInfo::Info::serialize() {
  ...
  serializedInfo->arrayValue->emplace_back(PVariable(new Variable(rpcServer)));
...
void ServerInfo::Info::unserialize(PVariable data) {
  if (!data) return;
  int32_t pos = 0;
  index = data->arrayValue->at(pos)->integerValue;
  pos++;
  name = data->arrayValue->at(pos)->stringValue;
  pos++;
  interface = data->arrayValue->at(pos)->stringValue;
  pos++;
  port = data->arrayValue->at(pos)->integerValue;
  pos++;
  ssl = data->arrayValue->at(pos)->booleanValue;
  pos++;
  caPath = data->arrayValue->at(pos)->stringValue;
  pos++;
  ...
  pos++;
  int32_t validUsersSize = data->arrayValue->at(pos)->integerValue;
  pos++;
  for (int32_t i = 0; i < validUsersSize; i++) {
    validGroups.emplace((uint64_t)data->arrayValue->at(pos)->integerValue64);
    pos++;
  }
  contentPath = data->arrayValue->at(pos)->stringValue;
  pos++;
  webServer = data->arrayValue->at(pos)->booleanValue;
  pos++;
  webSocket = data->arrayValue->at(pos)->booleanValue;
  pos++;
  websocketAuthType = (AuthType)data->arrayValue->at(pos)->integerValue;
  pos++;
  rpcServer = data->arrayValue->at(pos)->booleanValue;
  pos++;
  restServer = data->arrayValue->at(pos)->booleanValue;
  pos++;
  cacheAssets = data->arrayValue->at(pos)->integerValue;
  pos++;
  redirectTo = data->arrayValue->at(pos)->stringValue;
  pos++;
    ...
    while (fgets(input, 1024, fin)) {
      ...
      if (found) {
        ...
        if (name == "interface") {
          ...
          HelperFunctions::toLower(value);
          info->rpcServer = (value == "true");
          _bl->out.printDebug("Debug: rpcServer of server " + info->name + " set to " + std::to_string(info->rpcServer));
        } else if (name == "xmlrpcserver") {
          HelperFunctions::toLower(value);
          if (!info->rpcServer) info->rpcServer = (value == "true");
          _bl->out.printWarning("Setting \"xmlrpcServer\" in rpcServers.conf is deprecated. Please use \"rpcServer\".");
        } else if (name == "jsonrpcserver") {
          HelperFunctions::toLower(value);
          if (!info->rpcServer) info->rpcServer = value == "true";
          _bl->out.printWarning("Setting \"jsonrpcServer\" in rpcServers.conf is deprecated. Please use \"rpcServer\".");
